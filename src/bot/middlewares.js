...
module.exports = (bot) => {
  bot.use(async (ctx, next) => {
    if (ctx.message) {
      ...
      console.log(`Recibido callbackQuery de ${ctx.from.username}: ${ctx.callbackQuery.data}`);
    }
...
const trackingMiddleware = async (ctx, next) => {
  ...
  if (ctx.from && ctx.from.id) {
    ...
    if (ctx.chat && ctx.chat.id) {
      await ctx.telegram.sendMessage(ctx.chat.id, 'Hubo un problema al procesar tu solicitud. Por favor, intenta nuevamente más tarde.');
    }
  }
};
